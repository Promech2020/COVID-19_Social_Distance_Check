...
class Ui_MainWindow(QWidget):
	...
	def setupUi(self, MainWindow):
		...
		self.alert_file_browser_button.clicked.connect(lambda: self.browse_files("Audio files (*.mp3)"))
	...
	def webcam_clicked(self):
		...
		self.okButton1.clicked.connect(lambda: self.ok_pressed(self.webcam_target_distance_user_input, self.webcam_target_distance))
	...
	def online_clicked(self):
		...
		self.okButton2.clicked.connect(lambda: self.ok_pressed(self.online_data_link_user_input, self.get_online_data))
		self.cancelButton2.clicked.connect(lambda: self.cancel_pressed(self.get_online_data))

	def ok_pressed(self, user_input, dialog ):
	...
	def proceed_processing(self):
		...
		if self.file_path == "" and self.audio_path=="" and self.webcam_center_target_distance=="":
			self.title = "No Info Provided"
			self.info = "You have not provided any info. We will proceed with our default."
			self.webcam_center_target_distance = "15"
